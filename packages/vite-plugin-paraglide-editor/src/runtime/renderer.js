...
export function renderTranslation(key, params = {}, locale) {
  const messageFunctions = window.__paraglideEditor?.messageFunctions;

  if (!messageFunctions) {
    console.warn('[paraglide-editor] Message functions not available, wrapper not initialized yet');
    return '';
  }

  const messageFunction = messageFunctions[key];
  if (!messageFunction) {
    console.warn(`[paraglide-editor] Message function not found: ${key}`);
    return '';
  }
  ...
  try {
    ...
    console.error(`[paraglide-editor] Error rendering translation ${key}:`, error);
    return '';
  }
}
...
export function renderEditedTemplate(template, params = {}, locale = 'en') {
  ...
  if (typeof template === 'string' && template.trim().startsWith('[{')) {
    try {
      ...
      console.warn('[paraglide-editor] Failed to parse variant:', e);
    }
  }
