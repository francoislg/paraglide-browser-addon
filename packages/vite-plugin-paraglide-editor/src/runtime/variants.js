...
function parseDeclarations(declarations = []) {
  return declarations.filter(decl => decl && typeof decl === 'string').map(decl => {
    ...
    if (decl.startsWith('local ')) {
      ...
      if (equalsPos === -1) {
        console.warn('[paraglide-editor] Invalid local declaration (missing =):', decl);
    ...
    console.warn('[paraglide-editor] Unknown declaration format:', decl);
    return { type: 'unknown', name: decl };
  });
}
...
export function renderVariant(variant, params = {}, locale = 'en') {
  if (!variant || !variant.match || typeof variant.match !== 'object') {
    console.warn('[paraglide-editor] Invalid variant: missing match object');
    return '';
  }
  ...
  if (!result.template) {
    console.warn('[paraglide-editor] No matching template found in variant');
