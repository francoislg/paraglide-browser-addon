...
export async function initConflictList() {
  try {
    ...
    let listContainer = document.getElementById('pge-conflict-list');
    if (!listContainer) {
      console.warn('[paraglide-editor] Conflict list container not found');
      return;
    }
    ...
    console.error('[paraglide-editor] Failed to load conflicts:', error);
  }
}

function createConflictItem(conflict) {
  const item = document.createElement('div');
  item.className = 'pge-conflict-item';
  item.style.cssText = `
...
async function showConflictResolution(conflict) {
  const existing = document.getElementById('pge-conflict-resolution');
  if (existing) {
    existing.remove();
  }

  const dialog = document.createElement('div');
  dialog.id = 'pge-conflict-resolution';
  dialog.classList.add('pge-ignore-detection');
  ...
  dialog.innerHTML = `
    ...
    ">
      ...
      <div style="display: flex; gap: 8px; margin-top: 24px; flex-wrap: wrap;">
        <button id="pge-keep-local" style="
          flex: 1;
          min-width: 120px;
          background: #90cdf4;
          color: #1a202c;
        ...
        <button id="pge-keep-server" style="
          flex: 1;
          min-width: 120px;
          background: #fbd38d;
          color: #1a202c;
        ...
        <button id="pge-merge-custom" style="
          flex: 1;
          min-width: 120px;
          background: #48bb78;
          color: white;
        ...
        <button id="pge-cancel-resolution" style="
          background: #718096;
          color: white;
          border: none;
          padding: 10px 20px;
  ...
  const keepLocalBtn = dialog.querySelector('#pge-keep-local');
  const keepServerBtn = dialog.querySelector('#pge-keep-server');
  const mergeCustomBtn = dialog.querySelector('#pge-merge-custom');
  const cancelBtn = dialog.querySelector('#pge-cancel-resolution');
  const mergeTextarea = dialog.querySelector('#pge-conflict-merge-value');
